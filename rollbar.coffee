initialize = do ->
  ...
  ({config, token, env}) ->
    ...
    _rollbarConfig = config ?
      ...
      payload:                    { environment: env or 'production' }
...
if (head = document.getElementsByTagName('head')[0])?
  ...
  for el in meta when el.getAttribute('name') in ['app:environment', 'environment', 'app:env']
    break if env = el.getAttribute('content')

  for el in meta
    name = el.getAttribute('name')
    if name is 'rollbar:config'
      try config = JSON.parse(el.getAttribute('content'))
      if config?
